...
<body class="">
	...
	<form id="form_list_params" style="display:none;">
		<input id="referer" type="text" value="1" name="referer" />
...
$(function(){
	...
	var pager = new common.customPage({
		startnow : true,
		url :  '${ctx}/base/hotel/applyHotelList',
...
<div class="one-list" style="">
	...
	<div class="font-size-min" style="margin-top:0.5rem;">
		<div style="float:left;">场地描述：{{item.description}}</div>
		<div style="clear:both;"></div>
	</div>
	<div class="one-list-ct-sett" style="margin-top:0.5rem;">
		{{if (item.applyState == '1') && (item.userId == '${aUs.getCurrentUserId()}') && ('${aUs.getCurrentUserType()}'=="CLIENT")}}
		  <input class="btn btn-xs bg-type-01" type="button" style="float:right;min-width:4.2rem;margin 0 auto;" id="id{{item.id}}" onclick="test({{item.id}})" value="撤回"/>
		{{else if (item.joinState == '1') && (item.userId == '${aUs.getCurrentUserId()}') }}
		  <input class="btn btn-xs bg-type-01" type="button" style="float:right;min-width:4.2rem;margin 0 auto;"  value="已加入"/>
		{{else if ('${aUs.getCurrentUserType()}'=="CLIENT")}}
		   <input class="btn btn-xs bg-type-01" type="button" style="float:right;min-width:4.2rem;margin 0 auto;" id="id{{item.id}}" onclick="test({{item.id}})" value="申请加入"/>
		{{/if}}
	</div>
	<div style="clear:both;"></div>
</div>
...
<script type="text/javascript">
	function test(id){
		var btn = document.getElementById("id"+id).value;
		if(btn=="申请加入"){
			$.post('${ctx}/base/hotel/record/saveRecord',{hotelId:id},function(res){
				if(res.statusCode==='200'){
					document.getElementById("id"+id).value = "撤回";
					toastFn("申请成功!",1500,null);
				}else{
				}
			},'json');
		}else if(btn=="撤回"){
			$.post('${ctx}/base/hotel/record/cancle',{hotelId:id},function(res){
				if(res.statusCode==='200'){
					document.getElementById("id"+id).value = "申请加入";
					toastFn("撤回成功!",1500,null);
				}else{
				}
			},'json');
		}
