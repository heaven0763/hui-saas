...
<body class="">
	...
	<div id="top_toolbar" class="toolbar" style="">
		<div class="toolbar-title" style="text-align: left;margin-left: 0.3rem;">
			...
			<div class="" style="display: table-cell;color: #999999;text-align: left;width:75%;vertical-align: middle;position: relative;">
				<span id="datetxt" style="text-align: left;"></span>
			</div> 
			<!-- <div class="toolbar-one" style="">
	...
	<form id="form_list_params" style="display:none;">
		...
		<input id="search_LIKE_hotelName" type="text" name="search_LIKE_o.hotel_name" />
		...
		<input id="search_fdate" type="text" name="search_GTE_o.commission_date"  />
		<input id="search_sdate" type="text" name="search_LTE_o.commission_date"  />
		
		<input id="search_state" type="text" name="search_EQ_o.commission_status" value = "07"  /> 
		<input id="" type="text" name="search_OR_EQ;o.state" value="06,07" /> 
		<input id="search_saleId" type="text" name="saleId" value="${groupMap.iscompanysales?guserId:'' }" /> 
	...
	*/
	$('#btn_date_selt').click(function(){
		if($("#fdate").val()==''&&$("#sdate").val()==''){
			...
			pager.search();	//查询
			$("#datetxt").text($("#sdate").val()+"~"+$("#fdate").val());
			checkAndQuit(0);
		}
	});
	//var date_selt = 
...
{{each rows as item i}}
<div class="one-list" style="" item-rdurl="${ctx}/weixin/order/deduct/detail/{{item.id}}">
	<div class="one-list-ct-title" style="">
		<div style="display:inline-block;">{{item.hotelName}}</div>
		<div class="list-ct-state list-ct-state-{{item.state}} " style="">{{item.state | tp_fn05}}</div>
		<div style="float:right;color:#cb2b29;">{{item.amount | order_amount}}</div>
	</div>
	<div class="font-size-min" style="margin-top:0.5rem;">订单号：{{item.orderNo}}</div>
	<div class="font-size-min" style="margin-top:0.5rem;">
		<div style="float:left;">活动时间：{{item.activityDate}}</div>
		{{item | commission}}
		<div style="clear:both;"></div>
	</div>
	<div class="one-list-ct-sett" style="margin-top:0.5rem;">
		<div class="sett-left" style="">地区：{{item.area}}</div>
		
		<div class="btn btn-xs bg-type-01" style="float:right;min-width:4.2rem;margin 0 auto;">查看详情</div>
	</div>
	<div style="clear:both;"></div>
</div>
{{/each}}
