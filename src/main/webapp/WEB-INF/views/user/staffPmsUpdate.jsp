 ...
 <link href="${ctx}/static/weixin/css/common.css" rel="stylesheet" />
    ...
    <style type="text/css">
    	...
    	.info-one-div,.eva-one-div{
    		border-top:1px solid #cccccc;margin-top:8px;font-size:14px;
    	}
...
<div  class="wrapper wrapper-content">
	<div class="row">
		<div class="col-sm-12" style="position: relative;">
			<a href="javascript:loadContent(this,'${ctx}/base/user/detail/${user.id}?TYPE=${TYPE}','')" class="btn btn-warning" style="position: absolute;right: 20px;margin-top: 5px;top: 7px;"><span class="glyphicon glyphicon-arrow-left"> </span> 返回</a>
			<h3>权限调整</h3>
		</div>
	</div>
   
	<div class="user_list_one" style="margin：0;">
		<!-- <div class="info-one-div" style="margin：0;">
			<div class="btn btn-xs bg-none-00" style="padding:0.2rem 1rem;font-size: 0.85rem;">基本信息</div>
		</div> -->
		<div class="row">
			<div class="col-sm-12" style="background-color:#048dd3;color: #FFF;padding: 5px;"><span style="margin-left: 2%;">基本信息</span></div>
		</div>
		<div style="line-height:1.3rem;padding:0 20px;">
			<div class="flex-box flex-justify" style="margin: 8px 0;line-height: 1.3rem;border-bottom: 1px solid #f5f5f5;padding: 3px 0;">
				<div style="">
				...
				<div>
					<a class="btn btn-primary btn-sm"  href="${ctx}/base/user/pwdForm/${user.id}" target="dialog"><span class="glyphicon glyphicon-cog"></span> 重置密码</a>
				</div>
			</div>
			...
			<div class="flex-box flex-justify" style="margin: 0.5rem 0;">
				<div>个人拥有权限：
					<button class="btn btn-primary btn-sm" id="pms_slf_sel_all" type="button">全选</button>
					<button class="btn btn-default btn-sm" id="pms_slf_unsel_all" type="button">全不选</button></div>
			</div>
	...
	<div class="flex-box flex-justify" style="background-color: #ffffff;padding: 0.5rem 2%;width:96%;">
	...
	<!-- <div id="other_group_pms_parent" class="div-tips-dialog" style="display:none;margin-top: 0%;top:10%;height:600px;width:600px;left: 35%;padding: 20px;">
...
function saveUserPermission(){	//保存
	...
	/* $('#permission_ids').val(permission_ids.join(','));
	 common.submitForm($('#form'),function(res){	//提交表单
		swal('保存成功！',"success");
		//loadContent(this,'${ctx}/base/user/staff/pms/update/${user.id}','RD');
		loadContent(this,'${ctx}/base/user/staff/pms/update/${user.id}?TYPE=${TYPE }','RD');
	},function(res){
		swal('保存失败！','error');
	}); */
	...
	common.ajaxjson({
		...
		success : function(res){
			...
			for(var i in allPermissions){
				...
				html.push('<div class="btn btn-xs btn-disable pms-item" pmsid="'+allPermissions[i].id + '" has="0" >'+allPermissions[i].displayname+'</div>');
			}
	...
	<div class="" style="border-bottom:0.1rem solid #cccccc;padding-bottom:0.5rem;">
		...
		<div class="other-groups-pms" style="border-top:0.1rem solid #cccccc;display:none;">
			...
			{{each item.permissions as pms j}}
				<div class="btn btn-xs {{pms.id | fm_has_pms}} pms-ren pms-ren-{{item.id}}" pmsid="{{pms.id}}" style="padding:0.2rem 0;width:48%;margin:0.2rem" >{{pms.displayname}}</div>
			{{/each}}
		</div>		
	</div>
{{/each}}
