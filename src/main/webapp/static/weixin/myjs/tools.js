...
var tipsDialog = function(options){
	var _self = {};
	_self.setting = $.extend({
		maskDiv : $('#mask-div'),
		tipsDiv : $('#div_tips_dialog'),
		css : {},
		html : '',
		msgArray: [],
		ok : function(){ },
	...
	_self.show = function(){
		_self.setting.maskDiv.show();
		_self.setting.tipsDiv.show();
		 window.addEventListener('touchmove',disabledScroll);
	};
	_self.hide = function(){
		_self.setting.maskDiv.remove();
		_self.setting.tipsDiv.remove();
		 window.removeEventListener('touchmove',disabledScroll);
	};
	_self.init = function(){
		if(_self.setting.maskDiv.length < 1){
			_self.setting.maskDiv = $('<div id="mask-div"></div>').appendTo('body');
			_self.setting.maskDiv.click(function(){
				_self.hide();
				_self.setting.hideCallBack();
			});
		}
		var marginTop = 0;
		if(_self.setting.tipsDiv.length < 1){
			_self.setting.tipsDiv = $('<div id="tipsDiv" class="div-tips-dialog"></div>').appendTo('body');
		}
		_self.setting.tipsDiv.css(_self.setting.css);
		var msgHtml = [];
		
		if(_self.setting.html){
			msgHtml.push(_self.setting.html);
		}else{
			for(var i in _self.setting.msgArray){
				var msg = _self.setting.msgArray[i];
				if(typeof msg == 'string'){
					marginTop++;
					msgHtml.push('<div>'+msg+'</div>');
				}
			}
		}
		
		
		_self.setting.tipsDiv.css('margin-top', (-marginTop * 8 ) + '%').html(msgHtml.join(''));
	};
	_self.init();
