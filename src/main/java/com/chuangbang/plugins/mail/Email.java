...
public class Email  {   
	...
	public static boolean sendMixedMail(MailInfo mailInfo,String attachment,String img) {
		...
		try {   //创建邮件
			...
			//正文
			MimeBodyPart text = new MimeBodyPart();
			text.setContent(mailInfo.getContent()+"<br/><img src='cid:aaa.jpg'>","text/html;charset=UTF-8");
	...
	public static void main(String[] args) throws IOException{  
		...
		model.put("nickname", "梁本风");
		model.put("projectName", "台风预警信");
		model.put("bpurl", "http://120.76.157.232:8080/chuangbang/static/uploadfiles/file/pdfs/20160624/2016-06-24-18-38-37-760416.pdf");
		model.put("url", "D:/order.jpg");
		model.put("load_date",DateTimeUtils.toSecond(new Date()));
		Template template = cfg.getTemplate("htmlmail.ftl");
		String content = FreeMarkers.renderTemplate(template, model);
		
		
		String title = "[创帮]趣玩星球项目商业方案";
		//这个类主要是设置邮件  
		MailInfo mailInfo = new MailInfo();   
		mailInfo.setMailServerHost("smtp.163.com");   
		mailInfo.setMailServerPort("25");   
		mailInfo.setValidate(true);   
		mailInfo.setUserName("haiwenandzilin@163.com");   
		mailInfo.setPassword("haiwen0822");//您的邮箱密码   
		mailInfo.setFromName("创帮");
		mailInfo.setFromAddress("haiwenandzilin@163.com");   
		mailInfo.setToAddress("596031785@qq.com");   
		mailInfo.setSubject(title);   
		mailInfo.setContent(content);    
		//这个类主要来发送邮件  
		//SimpleMailSender.sendTextMail(mailInfo);//发送文体格式   System.getProperty(Constant.WORKDIR)+
		Boolean bol = SimpleMailSender.sendMail(mailInfo,"html","","D:/order.pdf");//发送html格式
	} 
